...
from bert_explainer import analyze_text as bert_analyze_text  # 匯入文字分析模型
...
# CORS 設定，讓前端可以跨來源存取此 API
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # 設定可接受的前端來源（此處允許所有來源）
...
async def analyze_text_api(request: TextAnalysisRequest):
    try:
        ...
        result = bert_analyze_text(request.text)  # 使用 BERT 分析輸入文字
